...
# This page corresponds to material in 
...
# It is not necessarily clear that it is worth writing code to handle all these 
 # cases since they might be rare enough. However, some give us an opportunity 
...
# and then end there ($). The parenthesis defines the group that we pass as \\1 
# to the replace regex.

# ...................Cases 2 and 4...................

# We can adapt this code slightly to handle case 2 as well which covers the 
 # entry 5'. Note that the 5' is left untouched by the code above. This is 
 # because the extra ' makes the pattern not match since we have to end with a 
 # 5 or 6. To handle case 2, we want to permit the 5 or 6 to be followed by no 
 # or one symbol for feet. So we can simply add '{0,1} after the ' to do this. 
 # We can also use the none or once special character ?. As we saw previously, 
# this is different from * which is none or more. We now see that this code also
...
# Note that here we only permit 5 and 6 but not 4 and 7. This is because heights
 ...
 # typed 5 or 6 really meant either 60 or 72 inches. However, heights of exactly
 # 4 or exactly 7 feet tall are so rare that, although we accept 84 as a valid 
 # entry, we assume that a 7 was entered in error.

# ...................Case 3...................

# We can use quantifiers to deal with  case 3. These entries are not matched 
# because the inches include decimals and our pattern does not permit this. We 
# need allow the second group to include decimals and not just digits. This means
# we must permit zero or one period . followed by zero or more digits. So we will
# use both ? and *. Also remember that for this particular case, the period needs
# to be escaped since it is a special character (it means any character except a
# line break).
...
# one to 1, and so on. To avoid having to write two separate operations for Zero
# and zero, One and one, etc., we can use the str_to_lower() function to make 
...
# and tries to convert as many strings as possible to a single format. Below is 
# a function that puts together the previous code replacements:
...
# Before that we need
# 1. Install and run library(english)
# 2. Go to Str6 Case Study 2- Reported Heights and copy not_inches_or_cm & problems

#<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<>>>>>>>>>>>>>>>>>>>>>>>>>>>>

# We need to add this library
...
#<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<>>>>>>>>>>>>>>>>>>>>>>>>>>>>
# Make sure that you have reported_heights data
library(dslabs)
